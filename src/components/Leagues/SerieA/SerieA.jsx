...
export const SerieA = () => {
    ...
    useEffect(() => {
        axios.get('https://api-football-v1.p.rapidapi.com/v3/standings?season=2022&league=135', {
            headers: {
    ...
    return (
        <div className={styles.tableContainer}>
            <h1 className={styles.tableTitle}>Serie A Table</h1>
                ...
                <tbody>
                    {teams.map(team => (
                        <tr key={team.team.id} className={styles.tableRow}>
                            ...
                            <td className={styles.tableCell + ' ' + styles.tableCellTeam}><Link to={`/serie-a/${team.team.id}`}>{team.team.name}</Link></td>
